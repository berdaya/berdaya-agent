...
# Python-driven tool session (e.g. Berdaya Agent terminal tool). A pre-set PYTHONPATH
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Berdaya-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Berdaya Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hermes command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
install_uv() {
    ...
    # Berdaya Agent owns its own uv at $HERMES_HOME/bin/uv.  Always install there —
...
# string clears that floor; anything below it is replaced with the Berdaya Agent-
...
check_node() {
    ...
    if [ -x "$HERMES_HOME/node/bin/node" ] && node_satisfies_build "$("$HERMES_HOME/node/bin/node" --version)"; then
        ...
        log_success "Node.js $("$HERMES_HOME/node/bin/node" --version) found (Berdaya-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Berdaya-managed Node $NODE_VERSION LTS..."
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Berdaya Agent install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Berdaya Agent itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Berdaya Agent behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up berdaya command..."

    if [ "$USE_VENV" = true ]; then
        BERDAYA_BIN="$INSTALL_DIR/venv/bin/berdaya"
        HERMES_BIN="$INSTALL_DIR/venv/bin/hermes"
        if [ ! -x "$BERDAYA_BIN" ] && [ -x "$HERMES_BIN" ]; then
            BERDAYA_BIN="$HERMES_BIN"
        fi
    else
        BERDAYA_BIN="$(which berdaya 2>/dev/null || which hermes 2>/dev/null || echo "")"
        HERMES_BIN="$BERDAYA_BIN"
        if [ -z "$BERDAYA_BIN" ]; then
            log_warn "berdaya not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$BERDAYA_BIN" ]; then
        log_warn "berdaya entry point not found at $BERDAYA_BIN"
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        PATH_COMMENT='# Berdaya Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Berdaya Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Berdaya Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Berdaya Agent will use the system browser."
        else
            case "$DISTRO" in
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Berdaya Agent to send/receive messages."
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Berdaya Agent for pip install"
...
# extract a tree MISSING the electron binary, so the `electron`->`Berdaya Agent` rename
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Berdaya-managed Node to PATH, so
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Berdaya Agent" ]; then
            app="$desktop_dir/release/linux-unpacked/Berdaya Agent"
        ...
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Berdaya Agent.app" \
            "$desktop_dir/release/mac/Berdaya Agent.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Berdaya Agent is damaged and can't be opened").
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Berdaya-managed Node
