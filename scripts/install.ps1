# ============================================================================
# Berdaya Agent Installer for Windows
# ============================================================================
...
param(
    ...
    # builds apps/desktop into a launchable Berdaya Agent.exe.
    #
    # Why opt-in:
    #   * Berdaya-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Berdaya Agent.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Berdaya Agent.exe
...
function Write-Banner {
    ...
    Write-Host "|             * Berdaya Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Berdaya AI.                 |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $BerdayaHome)) {
        New-Item -ItemType Directory -Force -Path $BerdayaHome | Out-Null
    }
    $envFile = Join-Path $BerdayaHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $BerdayaHome "node"
...
function Install-Uv {
    # Berdaya Agent owns its own uv at $BerdayaHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `hermes update` stay in sync.
    $managedUv = Join-Path $BerdayaHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $BerdayaHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $BerdayaHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $BerdayaHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $BerdayaHome "bin"
...
function Install-Git {
    ...
    which Berdaya Agent uses to run shell commands.
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Berdaya Agent needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Berdaya Agent copy of Git ourselves is predictable and
    ...
    ``HERMES_GIT_BASH_PATH`` (User scope) so Berdaya Agent can find it in a fresh
    ...
    # Download PortableGit into $BerdayaHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $BerdayaHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Berdaya Agent features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Berdaya Agent needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HERMES_GIT_BASH_PATH`` (User env scope) so Berdaya Agent can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $BerdayaHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $BerdayaHome\git\usr\bin\bash.exe
    $candidates += "$BerdayaHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$BerdayaHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    Write-Warn "Could not locate bash.exe -- Berdaya Agent may not find Git Bash."
...
function Test-Node {
    ...
    # Prefer a Berdaya-managed Node from a previous run over a too-old system one.
    $managedNode = "$BerdayaHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$BerdayaHome\node;$env:Path"
        Write-Success "Node.js $version found (Berdaya-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Berdaya-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $BerdayaHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $BerdayaHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                if (Test-Path "$BerdayaHome\node") { Remove-Item -Recurse -Force "$BerdayaHome\node" }
                Move-Item $extractedDir.FullName "$BerdayaHome\node"
                ...
                $version = & "$BerdayaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $BerdayaHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Berdaya Agent behaves unexpectedly."
                        } else {
...
function Set-PathVariable {
    Write-Info "Setting up berdaya command..."
...
function Write-BootstrapMarker {
    # Writes $InstallDir\.hermes-bootstrap-complete which tells the Berdaya Agent
    ...
    # Berdaya-Setup.exe) or fall back to whatever git resolves in the
...
function Copy-ConfigTemplates {
    ...
    # Create the HERMES_HOME directory structure ($BerdayaHome, default %LOCALAPPDATA%\hermes)
    New-Item -ItemType Directory -Force -Path "$BerdayaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BerdayaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BerdayaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BerdayaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BerdayaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BerdayaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BerdayaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BerdayaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$BerdayaHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$BerdayaHome\.env"
    ...
    # Create config.yaml
    $configPath = "$BerdayaHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Berdaya Agent's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$BerdayaHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        $soulContent = @"
# Berdaya Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Berdaya Agent communicates with you.
...
"@
    ...
    Write-Success "Configuration directory ready: $BerdayaHome"
    
    # Seed bundled skills into $BerdayaHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $BerdayaHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $BerdayaHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$BerdayaHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $BerdayaHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Berdaya-Setup.exe runs each -Stage NAME
...
# the final `electron` -> `Berdaya Agent` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Berdaya Agent.exe. Only called from
    ...
    # resolves apps/desktop/release/win-unpacked/Berdaya Agent.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Berdaya-managed
    ...
    # The --dir mode produces an unpacked Berdaya Agent.exe in
    ...
    # are NOT trying to work around). The Berdaya Agent icon + product name are
    # stamped onto Berdaya Agent.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    try {
        ...
        if ($code -ne 0) {
            ...
            # ENOENT on the final `electron` -> `Berdaya Agent` rename: app-builder's
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Berdaya Agent.exe",
        "$desktopDir\release\win-arm64-unpacked\Berdaya Agent.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Berdaya Agent.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Berdaya Agent icon + identity are stamped onto Berdaya Agent.exe by the
    ...
    #    Berdaya Agent.exe. We deliberately do NOT point them at `hermes desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Berdaya Agent.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Berdaya Agent.lnk')
        )
        ...
        # with the Berdaya Agent icon, but without this the shortcut can keep drawing
...
function Start-GatewayIfConfigured {
    ...
    $whatsappSession = "$BerdayaHome\whatsapp\session\creds.json"
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$BerdayaHome\logs\gateway.log"
            Start-Process -FilePath $hermesCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$BerdayaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
...
function Write-Completion {
    ...
    Write-Host "$BerdayaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$BerdayaHome\.env"
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Berdaya Agent repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Berdaya-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Berdaya Agent to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
