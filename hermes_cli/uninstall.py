...
def remove_path_from_shell_configs():
    """Remove Berdaya Agent PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Berdaya Agent" comment and following line
                if '# Berdaya Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    the FHS fix).  Only symlinks that resolve into this Berdaya Agent home's ``node``
...
def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip Berdaya-owned entries from User-scope PATH in the registry.
...
def run_gui_uninstall(args):
    ...
    print(color("│         ⚕ Berdaya Agent Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Berdaya Agent Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Berdaya Agent agent stays installed.", Colors.CYAN))
    print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Berdaya Agent agent is still installed. Run 'hermes' to use the CLI,")
...
def run_uninstall(args):
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Berdaya Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Berdaya Agent data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Berdaya Agent code but keep your configuration and data.")
    ...
    *,
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Berdaya-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No Berdaya-set User env vars to remove")
    ...
    #     (only when they still point into this Berdaya Agent home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Berdaya-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No Berdaya-managed node/npm/npx symlinks found")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/berdaya/berdaya-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/berdaya/berdaya-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Berdaya Agent! ⚕")
    print()
