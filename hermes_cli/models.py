...
def _xai_curated_models() -> list[str]:
    ...
    when the cache is empty or unreadable. Berdaya Agent refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Berdaya Agent that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Berdaya Agent that doesn't ship
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("berdaya-cloud",   "Berdaya Cloud",            "Berdaya AI — production API at api.berdaya.ai"),
    ProviderEntry("berdaya-local",   "Berdaya Local",            "Berdaya AI — local dev server at 127.0.0.1:8000"),
...
#
# Some vendors expose several Berdaya Agent provider slugs (one per endpoint /
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Berdaya Agent' canonical provider ids.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Berdaya Agent should expose the /fast toggle for this model."""
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Berdaya Agent release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Berdaya Agent release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Berdaya Agent release.
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Berdaya Agent' default Claude IDs elsewhere use
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Berdaya Agent will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Berdaya Agent cannot verify the model name."
