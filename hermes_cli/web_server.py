...
app = FastAPI(title="Berdaya Agent", version=__version__, lifespan=_lifespan)
...
_SESSION_HEADER_NAME = "X-Berdaya-Session-Token"
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Berdaya-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Berdaya Agent (no terminal prompt), "
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Berdaya Agent provider slug — fine.
    ...
    1. Vendor-name → Berdaya-provider mapping: when the provider string is
       not a known Berdaya Agent provider/alias (e.g. ``moonshotai``, ``x-ai`` is
...
async def check_hermes_update(force: bool = False):
    """Report whether a Berdaya Agent update is available, without applying it.
    ...
    Returns:
        ...
        current_version: installed Berdaya Agent version string
...
async def search_sessions(q: str = "", limit: int = 20):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Berdaya Agent surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Berdaya Agent supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Berdaya Agent from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Berdaya Agent to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Berdaya Agent from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Berdaya Agent to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Berdaya Agent in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Berdaya Agent through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Berdaya Agent via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Berdaya Agent through an IMAP/SMTP mailbox.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Berdaya Agent to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Berdaya Agent inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Berdaya Agent through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Berdaya Agent to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Berdaya Agent to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Berdaya Agent as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Berdaya Agent").strip() or "Berdaya Agent"
...
def _restart_gateway_after_telegram_onboarding() -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Berdaya Agent appears
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.hermes/.anthropic_oauth.json`` — Berdaya-managed PKCE flow (what
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Berdaya Agent PKCE ({_HERMES_OAUTH_FILE})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Berdaya Agent even
    when they also have a separate Berdaya-managed PKCE login.
    """
    try:
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Berdaya-managed PKCE file
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Berdaya Agent file AND credential pool.
...
_SKILL_HUB_SOURCE_LABELS = {
    ...
    "hermes-index": "Berdaya Agent Index",
    "skills-sh": "skills.sh",
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Berdaya Agent inside WSL2 to use the dashboard's /chat "
...
def mount_spa(application: FastAPI):
    ...
    # (the MC Pages app), not the Berdaya Agent backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Berdaya Agent Teal",         "description": "Classic dark teal — the canonical Berdaya Agent look"},
    {"name": "default-large", "label": "Berdaya Agent Teal (Large)", "description": "Berdaya Agent Teal with bigger fonts and roomier spacing"},
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Berdaya Agent exposes a
    ...
    print(f"  Berdaya Agent Web UI → http://{host}:{port}")
