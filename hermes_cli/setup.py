"""
Interactive setup wizard for Berdaya Agent.
...
_DOCS_BASE = "https://github.com/berdaya/berdaya-agent"
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Berdaya Agent Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Berdaya Agent using environment variables or config commands:")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Berdaya Agent auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Berdaya Agent runs shell commands and code.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Berdaya Agent delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://github.com/berdaya/berdaya-agent")
    print()
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name="Berdaya Agent",
            bot_description="Your Berdaya Agent agent on Slack",
        )
        ...
        print_info(
            ...
            "Berdaya Agent adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Berdaya Agent delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Berdaya Agent to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print_info("   https://github.com/berdaya/berdaya-agent")
    print()
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Berdaya Agent from anywhere.")
...
# Config values may have different semantics between OpenClaw and Berdaya Agent.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Berdaya Agent to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Berdaya Agent at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Berdaya Agent at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Berdaya Agent at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Berdaya Agent at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Berdaya Agent equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Berdaya Agent config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Berdaya Agent.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Berdaya Agent's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Berdaya Agent can preview what would be imported before making any changes.")
    print()
    ...
    # Execute the migration — overwrite=False so existing Berdaya Agent configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Berdaya Agent config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Berdaya Agent (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Berdaya Agent session — model selected, provider set, and web/image/tts/browser
    ...
    print(color("│     ⚕ Berdaya Agent Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Berdaya Agent Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Berdaya Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Berdaya Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Berdaya Agent configured.")
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Berdaya Agent to messaging apps to chat from anywhere.")
