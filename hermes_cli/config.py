"""
Configuration management for Berdaya Agent.
...
#   the next ``subprocess.run([...])`` Berdaya Agent makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Berdaya Agent itself starts
...
#   ``HERMES_ENV`` — Berdaya Agent runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Berdaya Agent runtime location — never via dashboard env writer.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Berdaya Agent runtime location "
...
def is_managed() -> bool:
    """Check if Berdaya Agent is running in package-manager-managed mode.
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Berdaya Agent was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Berdaya Agent lives in a ``uv tool`` layout.
    ...
    the *active* Berdaya Agent is a regular pip/venv install, causing
...
#     — that script installs a *new* host-side Berdaya Agent, it doesn't update
...
Berdaya Agent runs as a published image (nousresearch/hermes-agent), not a
...
def format_managed_message(action: str = "modify this Berdaya Agent installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Berdaya Agent installation is managed by NixOS "
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Berdaya Agent installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
    ...
    return (
        f"Cannot {action}: this Berdaya Agent installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Berdaya Agent."
    )
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running Berdaya Agent commonly set these to map the in-container
...
def _is_container() -> bool:
    ...
    When Berdaya Agent runs in a container with volume-mounted config files, forcing
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default PROJECT.md into HERMES_HOME if the user doesn't have one yet."""
    from hermes_cli.default_soul import LEGACY_DEFAULT_SOUL_MD
    from hermes_cli.project_md import (
        DEFAULT_PROJECT_MD,
        LEGACY_SOUL_MD,
        PROJECT_MD,
        identity_path_for_read,
    )

    existing = identity_path_for_read(home)
    if existing is None:
        project_path = home / PROJECT_MD
        project_path.write_text(DEFAULT_PROJECT_MD, encoding="utf-8")
        _secure_file(project_path)
        return

    if existing.name != LEGACY_SOUL_MD:
        return
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Berdaya Agent-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Berdaya Agent
        ...
        # app, ACP) in a code workspace, Berdaya Agent adds a coding operating brief
    ...
    "terminal": {
        ...
        # skipped. When empty, Berdaya Agent auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Berdaya Agent sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Berdaya Agent
        ...
        # bundled Berdaya Agent image, which drops to the `hermes` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Berdaya Agent sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Berdaya Agent should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # single read_file page exceeds these limits, Berdaya Agent truncates the
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Berdaya Agent appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
    ...
    # response. If not, Berdaya Agent feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Berdaya Agent auto-pauses the goal and
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Berdaya Agent*" header.
    ...
    "security": {
        ...
        "acked_advisories": [],
        # Allow Berdaya Agent to lazy-install opt-in backend packages from PyPI
    ...
    #
    # Core Berdaya Agent tools (terminal, read_file, write_file, patch,
    ...
    "gateway": {
        ...
        # behavior: files must live under the Berdaya Agent cache, under
        ...
        # uploaded as native gateway attachments. Files inside the Berdaya Agent
...
OPTIONAL_ENV_VARS = {
    ...
    "BERDAYA_API_KEY": {
        "description": "Berdaya AI API key (Cloud and Local inference)",
        "prompt": "Berdaya API key",
        "url": "https://berdaya.ai/keys",
        "password": True,
        "category": "provider",
    },
    "BERDAYA_CLOUD_BASE_URL": {
        "description": "Berdaya Cloud API base URL override",
        "prompt": "Berdaya Cloud base URL (leave empty for api.berdaya.ai)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    },
    "BERDAYA_LOCAL_BASE_URL": {
        "description": "Berdaya Local API base URL override",
        "prompt": "Berdaya Local base URL (leave empty for 127.0.0.1:8000)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Berdaya Agent auth store)",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Berdaya Agent API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Berdaya Agent API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Berdaya Agent API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Berdaya Agent versions) write ``models`` as
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Berdaya Agent won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Berdaya Agent env var names, avoiding false positives from values
...
def reload_env() -> int:
    ...
    the .env file (but only vars known to Berdaya Agent — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Berdaya Agent vars that are no longer in .env
...
def show_config():
    ...
    print(color("│              ⚕ Berdaya Agent Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
