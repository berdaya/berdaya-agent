...
export const zh: Translations = {
  ...
  boot: {
    ...
    errors: {
      ...
      backgroundExitedDuringStartup: 'Berdaya Agent 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 Berdaya Agent Desktop。',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 Berdaya Agent 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      providers: '提供方',
    ...
    searchPlaceholder: {
      about: '关于 Berdaya Agent Desktop',
      config: '搜索设置…',
      gateway: '网关连接…',
      keys: '搜索 API 密钥…',
      mcp: '搜索 MCP 服务器…',
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 Berdaya Agent 跟随系统设置。',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: '当 Berdaya Agent 需要本地时间上下文时使用。留空则使用系统时区。',
      agent: {
        imageInputMode: '控制图片附件如何发送给模型。',
        maxTurns: 'Berdaya Agent 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: 'Berdaya Agent 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: 'Berdaya Agent 如何处理需要显式审批的命令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Berdaya Agent 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    about: {
      ...
      automaticUpdatesDesc: 'Berdaya Agent 会在后台自动检查更新，并在有可用更新时通知你。',
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Berdaya Agent Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Berdaya Agent 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 Berdaya Agent 后端。这是默认方式，并且可离线工作。',
      remoteTitle: '远程网关',
      remoteDesc:
        '将此桌面外壳连接到远程 Berdaya Agent 后端。托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Berdaya Agent Desktop 将使用已保存设置重新连接。',
      savedMessage: '已保存，下一次重启生效。',
      connectedTo: (baseUrl, version) => `已连接到 ${baseUrl}${version ? ` · Berdaya Agent ${version}` : ''}`,
    ...
    providers: {
      ...
      intro: '使用订阅登录，无需复制 API 密钥。Berdaya Agent 会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接，然后把 Berdaya Agent 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 Berdaya Agent 自带的 WhatsApp 桥接，首次运行时扫描二维码，然后启用该平台。',
      bluebubbles: '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 Berdaya Agent 指向该 URL。',
      ...
      weixin: '登录微信公众平台，复制 AppID 和 Token，并把消息回调 URL 指向 Berdaya Agent。',
      qqbot: '在 QQ 开放平台 (q.qq.com) 注册一个应用，复制 App ID 和 Client Secret。',
      api_server:
        '把 Berdaya Agent 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥，然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  profiles: {
    ...
    skillsLabel: '技能',
    notSet: '未设置',
    projectDesc: '内置于此项目的系统提示词与指令（PROJECT.md）。',
    descriptionLabel: '项目描述',
    descriptionOptional: '可选',
    descriptionPlaceholder: '这个项目是做什么的？',
    unsavedChanges: '有未保存的更改',
    loadingProject: '正在加载 PROJECT.md…',
    emptyProject: '空的 PROJECT.md —— 开始撰写项目说明…',
    saving: '保存中…',
    saveProject: '保存 PROJECT.md',
    deleteTitle: '删除项目？',
    deleteDescPrefix: '这将删除 ',
    deleteDescMid: ' 并移除其 ',
    deleteDescSuffix: ' 目录。此操作无法撤销。',
    deleting: '删除中…',
    createDesc: '项目是相互独立的 Berdaya Agent 工作区：各自拥有独立的配置、技能和 PROJECT.md。',
    nameLabel: '项目名称',
    cloneFromDefault: '从默认项目克隆',
    cloneFromDefaultDesc: '从你的默认项目复制配置、技能和 PROJECT.md。',
    invalidName: hint => `名称无效。${hint}`,
    nameRequired: '项目名称为必填项。',
    creating: '创建中…',
    createAction: '创建项目',
    renameTitle: '重命名项目',
    renameDescPrefix: '重命名会更新项目目录以及 ',
  ...
  cron: {
    ...
    newCron: '新建定时任务',
    emptyDescNew: '按 cron 表达式排程一个提示词。Berdaya Agent 会运行它，并把结果发送到你选择的目的地。',
  ...
  composer: {
    ...
    placeholderStarting: '正在启动 Berdaya Agent…',
    placeholderReconnecting: '正在重新连接 Berdaya Agent…',
    placeholderFollowUp: '发送后续消息',
    newSessionPlaceholders: [
      '我们要构建什么？',
      '给 Berdaya Agent 一个任务',
      '你在想什么？',
      '描述你需要什么',
      '我们该处理什么？',
      '随便问点什么',
    ...
    attachUrlDesc: 'Berdaya Agent 将抓取该页面并作为本回合的上下文。',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 Berdaya Agent 无法在应用内自行更新。',
    connectionRetry: '请检查网络连接后重试。',
    latestBody: '你正在运行最新版本。',
    latestBodyBackend: '后端正在运行最新版本。',
    allSetTitle: '已是最新',
    availableTitle: '有可用更新',
    availableBody: '新版 Berdaya Agent 已可安装。',
    availableTitleBackend: '后端有可用更新',
    availableBodyBackend: '已连接的 Berdaya Agent 后端有新版本可安装。',
    availableBodyNoChangelog: '已有新版本可用。此安装方式无法显示更新日志。',
    updateNow: '立即更新',
    maybeLater: '稍后再说',
    moreChanges: count => `另有 ${count} 项更改。`,
    manualTitle: '从终端更新',
    manualBody: '你是从命令行安装的 Berdaya Agent，因此更新也需要在那里运行。请将此命令粘贴到终端：',
    manualPickedUp: '下次启动 Berdaya Agent 时会使用新版本。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody: 'Berdaya Agent 更新器会在自己的窗口中接管，并在完成后重新打开 Berdaya Agent。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 Berdaya Agent 会自动重新连接。',
    applyingClose: 'Berdaya Agent 将关闭以应用更新。',
    errorTitle: '更新未完成',
  ...
  install: {
    ...
    oneTimeTitle: 'Berdaya Agent 需要一次性安装',
    ...
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 Berdaya Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某个安装步骤失败。在 Windows 上，如果另一个 Berdaya Agent CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 Berdaya Agent 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    activeDesc: '这是一次性设置。Berdaya Agent 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。',
  ...
  onboarding: {
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自托管',
        description: '将 Berdaya Agent 指向本地或自托管的 OpenAI 兼容端点 (vLLM、llama.cpp、Ollama 等)。'
      }
    },
    backToSignIn: '返回登录',
    getKey: '获取密钥',
    ...
    flowSubtitles: {
      ...
      device_code: '在浏览器中打开验证页面 — Berdaya Agent 会自动连接',
      loopback: '打开浏览器登录 — Berdaya Agent 会自动连接',
    ...
    authorizeThere: '请在那里授权 Berdaya Agent。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 Berdaya Agent，连接会自动完成，无需复制或粘贴。`,
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayOffline: '离线',
      gatewayTitle: 'Berdaya Agent 推理网关状态',
      agents: '代理',
      closeAgents: '关闭代理',
      openAgents: '打开代理',
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。Berdaya Agent 只会显示前 512 KB。`,
    previewAnyway: '仍然预览',
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: 'Berdaya Agent 正在重启...',
      askRestart: '让 Berdaya Agent 重启服务器',
      lookingRestart: taskId => `Berdaya Agent 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: 'Berdaya Agent 正在后台工作。可在预览控制台查看进度。',
      ...
      finishedRestarting: message => `Berdaya Agent 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: 'Berdaya Agent 无法重启服务器。',
      stillWorking: 'Berdaya Agent 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Berdaya Agent 正在加载回复',
      thinking: '思考中',
      today: time => `今天，${time}`,
    ...
    approval: {
      gatewayDisconnected: 'Berdaya Agent 网关未连接',
      sendFailed: '无法发送审批响应',
      run: '运行',
      moreOptions: '更多审批选项',
      allowSession: '允许本会话',
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.hermes/config.yaml)。Berdaya Agent 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: 'Berdaya Agent 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
    sudoPlaceholder: 'sudo 密码',
    secretTitle: '需要密钥',
    secretDesc: 'Berdaya Agent 需要一个凭据才能继续。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重启 Berdaya Agent 桌面版后可使用保存图片。',
    restartToSaveImages: '重启 Berdaya Agent 桌面版以保存图片',
