...
export const zhHant = defineLocale({
  ...
  boot: {
    ready: 'Berdaya Agent Desktop 已就緒',
    ...
    errors: {
      ...
      backgroundExitedDuringStartup: 'Berdaya Agent 背景程序在啟動期間結束。',
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed: '桌面後端拒絕了該請求 (405 Method Not Allowed)。請嘗試重新啟動 Berdaya Agent Desktop。',
  ...
  settings: {
    ...
    resetConfirm: '要將所有設定恢復為 Berdaya Agent 預設值嗎？',
    exportFailed: '匯出失敗',
    resetFailed: '重設失敗',
    nav: {
      providers: '提供方',
    ...
    searchPlaceholder: {
      about: '關於 Berdaya Agent Desktop',
      config: '搜尋設定…',
      gateway: '閘道連線…',
      keys: '搜尋 API 金鑰…',
      mcp: '搜尋 MCP 伺服器…',
    ...
    appearance: {
      ...
      colorMode: '色彩模式',
      colorModeDesc: '選擇固定模式，或讓 Berdaya Agent 跟隨系統設定。',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: 'Berdaya Agent 需要本機時間上下文時使用。留空則使用系統時區。',
      agent: {
        imageInputMode: '控制圖片附件如何傳送給模型。',
        maxTurns: 'Berdaya Agent 停止一次執行前的工具呼叫輪次上限。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Berdaya Agent 單次檔案讀取可讀取的最大字元數。',
      approvals: {
        mode: 'Berdaya Agent 如何處理需要明確批准的指令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Berdaya Agent 從應用程式內更新自身時，保留本機原始碼變更（stash）或丟棄（discard）。終端機更新一律會詢問。'
      }
    }),
    about: {
      ...
      automaticUpdatesDesc: 'Berdaya Agent 會在背景自動檢查更新，並在有可用更新時通知你。',
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'Berdaya Agent Desktop 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 Berdaya Agent 後端，請使用遠端閘道。在下方按設定檔指定各自的遠端主機。',
      appliesTo: '套用至',
      allProfiles: '全部設定檔',
      ...
      localTitle: '本機閘道',
      localDesc: '在 localhost 啟動私有 Berdaya Agent 後端。這是預設方式，可離線使用。',
      remoteTitle: '遠端閘道',
      remoteDesc:
        '將此桌面殼層連線至遠端 Berdaya Agent 後端。託管閘道使用 OAuth 或帳號密碼；自託管閘道也可使用工作階段 Token。',
      ...
      savedTitle: '閘道設定已儲存',
      restartingMessage: 'Berdaya Agent Desktop 將使用已儲存的設定重新連線。',
      savedMessage: '已儲存，下次重新啟動後生效。',
      connectedTo: (baseUrl, version) => `已連線至 ${baseUrl}${version ? ` · Berdaya Agent ${version}` : ''}`,
    ...
    providers: {
      ...
      intro: '使用訂閱登入，無需複製 API 金鑰。Berdaya Agent 會在應用程式中為您完成瀏覽器登入。',
      connected: '已連線',
      collapse: '收合',
  ...
  profiles: {
    ...
    title: '專案',
    count: count => `${count} 個專案`,
    loading: '正在載入專案…',
    newProfile: '新增專案',
    allProfiles: '全部專案',
    showAllProfiles: '顯示全部專案',
    ...
    skillsLabel: '技能',
    notSet: '未設定',
    projectDesc: '內建於此專案的系統提示詞與指令（PROJECT.md）。',
    descriptionLabel: '專案描述',
    descriptionOptional: '選填',
    descriptionPlaceholder: '這個專案是做什麼的？',
    unsavedChanges: '有未儲存的變更',
    loadingProject: '正在載入 PROJECT.md…',
    emptyProject: '空的 PROJECT.md — 開始撰寫專案說明…',
    saving: '儲存中…',
    saveProject: '儲存 PROJECT.md',
    deleteTitle: '刪除專案？',
    deleteDescPrefix: '這將刪除 ',
    deleteDescMid: ' 並移除其 ',
    deleteDescSuffix: ' 目錄。此操作無法復原。',
    deleting: '刪除中…',
    createDesc: '專案是獨立的 Berdaya Agent 工作區：各自擁有獨立的設定、技能和 PROJECT.md。',
    nameLabel: '專案名稱',
    cloneFromDefault: '從預設專案複製設定',
    cloneFromDefaultDesc: '從您的預設專案複製設定、技能和 PROJECT.md。',
    invalidName: hint => `專案名稱無效。${hint}`,
    nameRequired: '專案名稱為必填',
    creating: '建立中…',
    createAction: '建立專案',
    renameTitle: '重新命名專案',
    renameDescPrefix: '重新命名會更新專案目錄以及 ',
  ...
  cron: {
    ...
    newCron: '新排程工作',
    emptyDescNew:
      '按 cron 表達式排程一個提示詞。Berdaya Agent 會執行它，並將結果傳送至您選擇的目的地。',
  ...
  composer: {
    ...
    placeholderStarting: '正在啟動 Berdaya Agent...',
    placeholderReconnecting: '正在重新連線至 Berdaya Agent…',
    placeholderFollowUp: '傳送後續訊息',
    newSessionPlaceholders: [
      '我們要建立什麼？',
      '給 Berdaya Agent 一個任務',
      '您在想什麼？',
      '描述您需要什麼',
      '我們該處理什麼？',
      '盡管問',
    ...
    attachUrlDesc: 'Berdaya Agent 將擷取該頁面並作為此回合的脈絡。',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 Berdaya Agent 無法在應用程式內自行更新。',
    connectionRetry: '請檢查網路連線後重試。',
    latestBody: '您正在執行最新版本。',
    latestBodyBackend: '後端正在執行最新版本。',
    allSetTitle: '已是最新版本',
    availableTitle: '有可用更新',
    availableBody: '新版 Berdaya Agent 已可安裝。',
    availableTitleBackend: '後端有可用更新',
    availableBodyBackend: '已連接的 Berdaya Agent 後端有新版本可安裝。',
    availableBodyNoChangelog: '已有新版本可用。此安裝方式無法顯示更新日誌。',
    updateNow: '立即更新',
    maybeLater: '稍後再說',
    moreChanges: count => `另有 ${count} 項變更。`,
    manualTitle: '從終端機更新',
    manualBody: '您是從命令列安裝的 Berdaya Agent，因此更新也需要在那裡執行。請將此指令貼到終端機：',
    manualPickedUp: '下次啟動 Berdaya Agent 時會使用新版本。',
    copy: '複製',
    copied: '已複製',
    done: '完成',
    applyingBody: 'Berdaya Agent 更新程式會在自己的視窗中接管，並在完成後重新開啟 Berdaya Agent。',
    applyingBodyBackend: '遠端後端正在套用更新並將重新啟動。恢復後 Berdaya Agent 會自動重新連線。',
    applyingClose: 'Berdaya Agent 將關閉以套用更新。',
    errorTitle: '更新未完成',
  ...
  install: {
    ...
    oneTimeTitle: 'Berdaya Agent 需要一次性安裝',
    ...
    failedTitle: '安裝失敗',
    settingUpTitle: '正在設定 Berdaya Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某個安裝步驟失敗。在 Windows 上，如果另一個 Berdaya Agent CLI 或桌面執行個體正在執行，可能會出現這種情況。請停止正在執行的 Berdaya Agent 執行個體後重試。可查看下方的詳細資訊或 desktop 記錄中的完整記錄。',
    activeDesc:
      '這是一次性設定。Berdaya Agent 安裝程式正在下載相依套件並設定您的電腦。之後啟動會略過此步驟。',
  ...
  onboarding: {
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自託管',
        description: '將 Berdaya Agent 指向本機或自託管的 OpenAI 相容端點（vLLM、llama.cpp、Ollama 等）。'
      }
    },
    backToSignIn: '返回登入',
    getKey: '取得金鑰',
    ...
    flowSubtitles: {
      ...
      device_code: '在瀏覽器中開啟驗證頁面 — Berdaya Agent 會自動連線',
      loopback: '開啟瀏覽器登入 — Berdaya Agent 會自動連線',
    ...
    authorizeThere: '請在那裡授權 Berdaya Agent。',
    copyAuthCode: '複製授權碼並貼到下方。',
    pasteAuthCode: '貼上授權碼',
    reopenAuthPage: '重新開啟授權頁面',
    autoBrowser: provider =>
      `已在瀏覽器中開啟 ${provider}。請在那裡授權 Berdaya Agent，連線會自動完成，無需複製或貼上。`,
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayOffline: '離線',
      gatewayTitle: 'Berdaya Agent 推論閘道狀態',
      agents: '代理',
      closeAgents: '關閉代理',
      openAgents: '開啟代理',
  ...
  preview: {
    ...
    largeTitle: '此檔案較大',
    largeBody: (label, size) => `${label} 大小為 ${size}。Berdaya Agent 只會顯示前 512 KB。`,
    previewAnyway: '仍然預覽',
    ...
    web: {
      ...
      failedToLoad: '預覽載入失敗',
      tryAgain: '重試',
      restarting: 'Berdaya Agent 正在重新啟動...',
      askRestart: '請 Berdaya Agent 重新啟動伺服器',
      lookingRestart: taskId => `Berdaya Agent 正在尋找要重新啟動的預覽伺服器 (${taskId})`,
      restartingTitle: '正在重新啟動預覽伺服器',
      restartingMessage: 'Berdaya Agent 正在背景執行。可在預覽主控台查看進度。',
      ...
      finishedRestarting: message =>
        `Berdaya Agent 已完成預覽伺服器重新啟動${message ? `：${message}` : ''}`,
      failedRestarting: message => `伺服器重新啟動失敗：${message}`,
      unknownError: '未知錯誤',
      restartedTitle: '預覽伺服器已重新啟動',
      reloadingNow: '正在重新載入預覽。',
      restartFailedTitle: '預覽重新啟動失敗',
      restartFailedMessage: 'Berdaya Agent 無法重新啟動伺服器。',
      stillWorking:
        'Berdaya Agent 仍在執行，但尚未收到重新啟動結果。伺服器指令可能正在前台執行。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Berdaya Agent 正在載入回覆',
      thinking: '思考中',
      today: time => `今天，${time}`,
    ...
    approval: {
      gatewayDisconnected: 'Berdaya Agent 閘道未連線',
      sendFailed: '無法傳送核准回應',
      run: '執行',
      moreOptions: '更多核准選項',
      ...
      alwaysDescription: pattern =>
        `這會將「${pattern}」模式加入永久允許清單（~/.hermes/config.yaml）。Berdaya Agent 對類似指令將不再詢問，包括目前工作階段和未來工作階段。`,
      alwaysAllow: '一律允許'
    },
    clarify: {
      notReady: '澄清請求尚未就緒',
  ...
  prompts: {
    ...
    sudoTitle: '管理員密碼',
    sudoDesc: 'Berdaya Agent 需要您的 sudo 密碼來執行特權指令。它只會傳送給您的本機代理。',
    sudoPlaceholder: 'sudo 密碼',
    secretTitle: '需要密鑰',
    secretDesc: 'Berdaya Agent 需要一個憑證才能繼續。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重新啟動 Berdaya Agent Desktop 後可使用儲存圖片。',
    restartToSaveImages: '重新啟動 Berdaya Agent Desktop 以儲存圖片',
