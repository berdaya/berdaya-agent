...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Berdaya Agent is ready', message: `${provider} connected.` })
}
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Berdaya Agent still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Berdaya Agent still cannot resolve a usable provider.'
}
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Berdaya Agent still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Berdaya Agent still cannot reach ${url}.` }
    }
