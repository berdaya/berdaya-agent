 ...
 *
 * A public install can end up with `origin=git@github.com:berdaya/berdaya-agent.git`.
...
const OFFICIAL_REPO_HTTPS_URL = 'https://github.com/berdaya/berdaya-agent.git'
const OFFICIAL_REPO_CANONICAL = 'github.com/berdaya/berdaya-agent'
// Pre-org-move installs may still carry the personal-fork remote.
const LEGACY_OFFICIAL_CANONICALS = new Set(['github.com/adrinant/berdaya-agent'])
...
function isOfficialSshRemote(url) {
  if (!isSshRemote(url)) return false
  const canonical = canonicalGitHubRemote(url)
  return canonical === OFFICIAL_REPO_CANONICAL || LEGACY_OFFICIAL_CANONICALS.has(canonical)
}
