...
// Seed the native About panel with the live Berdaya Agent version. This is refreshed
...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 Berdaya AI'
})
...
let bootProgressState = {
  ...
  message: 'Waiting to start Berdaya Agent backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function findSystemPython() {
  ...
  // CPython but several Berdaya Agent deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Berdaya Agent' terminal tool requires
...
// `hermes update`). Instead we spawn the staged Berdaya-Setup binary with
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Berdaya Agent updater…', percent: 100 })
...
async function applyUpdatesPosixInApp() {
  ...
  // Put the Berdaya-managed Node and the venv on PATH so `hermes desktop`'s
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Berdaya Agent (git + dependencies)…', percent: 10 })
  ...
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Berdaya Agent to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Berdaya Agent.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Berdaya Agent.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Berdaya Agent to load the new version.',
      percent: 100
    })
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Berdaya Agent to load the new version.',
      percent: 100
    })
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Berdaya Agent at ${ACTIVE_HERMES_ROOT}`,
...
function resolveHermesBackend(dashboardArgs) {
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Berdaya Agent CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      rememberLog(
        `Ignoring existing Berdaya Agent CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  return {
    ...
    label: 'Berdaya Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Berdaya Agent install found; starting first-launch bootstrap')
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Berdaya Agent install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Berdaya Agent bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
  ...
  // On Windows, preflight Git Bash. Berdaya Agent' terminal tool calls bash.exe
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Berdaya Agent on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Berdaya Agent.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `Berdaya Agent venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Berdaya Agent at ${ACTIVE_HERMES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Berdaya Agent runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Berdaya Agent backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Berdaya Agent backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Berdaya Agent backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Berdaya-Session-Token`` header — used by the auth-mode probe before
...
async function waitForHermes(baseUrl, token) {
  ...
  throw new Error(`Berdaya Agent backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
// signal so the chat composer doesn't stay stuck on "Starting Berdaya Agent...".
...
//
// Hosted Berdaya Agent gateways gate the dashboard behind an OAuth provider (e.g.
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Berdaya Agent gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Berdaya Agent backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Berdaya Agent gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Berdaya Agent gateway is selected, but no session token is saved. ' +
    ...
    if (!rawEnvToken) {
      throw new Error(
        'HERMES_DESKTOP_REMOTE_URL is set but HERMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Berdaya Agent backend.'
      )
    }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Berdaya Agent
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  // connect to Berdaya Agent gateway". Mirror the renderer's connect here so the test
...
async function spawnPoolBackend(profile, entry) {
  ...
  child.once('error', error => {
    rememberLog(`Berdaya Agent backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Berdaya Agent backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Berdaya Agent backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
...
async function startHermes() {
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Berdaya Agent backend', 8)
    ...
    rememberLog(`Starting Berdaya Agent backend via ${backend.label}`)
    ...
    hermesProcess.once('error', error => {
      rememberLog(`Berdaya Agent backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Berdaya Agent backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      rememberLog(`Berdaya Agent backend exited (${signal || code})`)
      hermesProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Berdaya Agent backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
    ...
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Berdaya Agent backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// "Starting Berdaya Agent…". Before the renderer's backoff loop reconnects, it asks us
...
ipcMain.handle('hermes:notify', (_event, payload) => {
  ...
  new Notification({
    title: payload?.title || 'Berdaya Agent',
    body: payload?.body || '',
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Berdaya Agent' TUI into
  ...
  env.TERM_PROGRAM = 'Berdaya Agent'
...
ipcMain.handle('hermes:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Berdaya Agent.')
  }
...
// Resolve the canonical Berdaya Agent version (the one `release.py` bumps in
...
// Re-resolve the live Berdaya Agent version and push it into the native About panel
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Berdaya Agent agent venv at ${VENV_ROOT}.`
    }
  }
