...
DEFAULT_AGENT_IDENTITY = (
    "You are Berdaya Agent, an intelligent AI assistant created by Berdaya AI. "
...
HERMES_AGENT_HELP_GUIDANCE = (
    "You run on Berdaya Agent (by Berdaya AI). When the user needs help with "
    "Berdaya Agent itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://github.com/berdaya/berdaya-agent is your "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Berdaya Agent "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Berdaya Agent WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Berdaya Agent itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Berdaya Agent runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Berdaya Agent itself is running. The host OS, home, and cwd "
                f"of the Berdaya Agent process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Berdaya Agent "
    ...
    # Berdaya Agent desktop GUI — any agent running under the desktop app should know
    ...
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Berdaya Agent desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Berdaya Agent
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Berdaya Agent itself — its CLI, config, models, providers, tools, "
