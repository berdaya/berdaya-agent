...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Berdaya Agent triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://github.com/berdaya/berdaya-agent",
    "X-Title": "Berdaya Agent",
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Berdaya Agent)",
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Berdaya Agent auth store.
